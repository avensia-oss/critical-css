...
function mapRule(node) {
    ...
    var items = [(0, css_tree_1.generate)(node.prelude, { mode: GEN_MODE }) + '{'];
    ...
    if (decl.value.type === 'Value') {
        ...
        decl.value.children.forEach(function (v) {
            ...
            if (v.type === 'String') {
                ...
                items.push(css_tree_1.string.encode(v.value));
            }
            else if (v.type === 'Url') {
                ...
                items.push((0, css_tree_1.generate)(v, { mode: GEN_MODE }));
            }
            else {
                items.push((0, css_tree_1.generate)(v, { mode: GEN_MODE }));
            }
        ...
        items.push((0, css_tree_1.generate)(decl.value, { mode: GEN_MODE }));
...
function pushUrlValue(items, node) {
    if (node.type === 'Url') {
        ...
        items.push((0, css_tree_1.generate)(node, { mode: GEN_MODE }));
    }
}
    ...
    switch (node.type) {
        case 'Atrule':
            if (node.name === 'supports' || node.name === 'media') {
                return {
                    type: 'media',
                    prelude: '@' + node.name + (node.prelude ? ' ' + (0, css_tree_1.generate)(node.prelude, { mode: "safe" }) : ''),
                ...
                node.prelude.children.forEach(function (c) {
                    if (isFirst_2 && c.type === 'String') {
                        ...
                        var unknown = (0, css_tree_1.generate)(c, { mode: GEN_MODE, });
                        items_1.push(
                ...
                node.block.children.forEach(function (c) {
                    ...
                    if (c.type === 'Declaration') {
                        ...
                        items_2.push((0, css_tree_1.generate)(c, { mode: GEN_MODE }));
                    }
                    first_1 = false;
                });
                items_2.push('}');
                ...
                var wat = [(0, css_tree_1.generate)(node.prelude, { mode: GEN_MODE }) + '{'];
                console.log({ wat: wat });
                node.prelude.children.forEach(function (c) {
                ...
                node.block.children.forEach(function (c) {
                    ...
                    if (c.type === 'Declaration') {
                        ...
                        items_3.push((0, css_tree_1.generate)(c, { mode: GEN_MODE }));
                    }
                    first_2 = false;
                });
                items_3.push('}');
                ...
                return (0, css_tree_1.generate)(node, { mode: GEN_MODE });
            }
        case 'Rule':
            return mapRule(node);
        case 'Raw':
